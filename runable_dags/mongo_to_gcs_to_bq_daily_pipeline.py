...
with DAG(
    ...
    def transform_orders(execution_date) -> None:
        #TODO: use Gemini to Generate code 
        #PROMPT: Read JSON data from GCS using gcs_client
        gcs_bucket_name = 'de-data-th-gemini'
        gcs_file_name = f'landing/orders/{execution_date.strftime("%Y-%m-%d")}.json'
    ...
    load_order = GCSToBigQueryOperator(
        ...
        source_objects=['transforming/order/*.parquet'],
        destination_project_dataset_table='gemini-nt-test-2.data_th_de_ecommerce.orders',
        source_format='PARQUET',
        write_disposition='WRITE_TRUNCATE',
        create_disposition='CREATE_IF_NEEDED',
    )

    load_order_item = GCSToBigQueryOperator(
        task_id='load_order_item',
        bucket='de-data-th-gemini',
        source_objects=['transforming/order_items/*.parquet'],
        destination_project_dataset_table='gemini-nt-test-2.data_th_de_ecommerce.order_items',
        source_format='PARQUET',
        write_disposition='WRITE_TRUNCATE',
        create_disposition='CREATE_IF_NEEDED',
    )
    ...
    extract_orders >> transform_orders >> [load_order, load_order_item]
